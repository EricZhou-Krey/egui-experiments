...
impl Navigator {
    fn draw_node_graph(&mut self, ui: &mut egui::Ui) {
        ...
        let screen_center = ui.content_rect().center();
        ...
        for x in -half_w..=half_w {
            for y in -half_h..=half_h {
                ...
                let intensity = 1.0 - (dist / self.spotlight_radius).powf(1.5);
                ...
                if intensity > 0.0 {
                    ...
                    let p_right = project(x_f + 1.0, y_f);
                    let p_down = project(x_f, y_f + 1.0);
...
impl Viewable for Navigator {
    ...
    /*fn draw_ui(&mut self, ui: &mut egui::Ui) {
        ...
        ui.scope_builder(UiBuilder::new().max_rect(ui.max_rect()), |graph_ui| {
            self.draw_node_graph(graph_ui)
        });
