use crate::{
    scene::{SceneObjectKey, scene_object::Shape}, settings::{
        MapSettings, logic_sheet::{
            MAP_BASE_ZOOM, MAP_ZOOM_LIMIT, MAP_ZOOM_SENSITIVITY }, style_sheet::{
    ...
    }, state::TTSState, terminal::{TTSTerminalExt, command::{
        AddSceneObjectCommand, ModifySceneObjectCommand, RemoveSceneObjectCommand,
        CommandFormatter, AddSceneObjectArgs, RemoveSceneObjectArgs, ModifySceneObjectArgs, ModifyAction
    }},
};
use glam::Vec2;
...
impl MapTool {
    ...
    pub fn interact(state: &mut TTSState, ui: &mut egui::Ui) {
        ...
        match state.map.map_selected_tool {
            ...
            MapTool::Select => {
                if response.clicked() && let Some(pointer_position) = response.interact_pointer_pos() {
                    ...
                    state.map.selected_object_key = state.scene.key_object_around(world_position, search_radius).map(|kv| kv.0);
                }
            }

            MapTool::Move => {
                if response.drag_started() && let Some(pointer_position) = response.interact_pointer_pos() {
                    let screen_position = Vec2::new(pointer_position.x, pointer_position.y);
                    let world_position = state.map.screen_to_world(screen_position);
                    let search_radius = state.map.settings.interaction_radius / state.map.zoom;
                    
                    if let Some(object_key) = state.scene.key_object_around(world_position, search_radius).map(|kv| kv.0) {
                        state.map.action_in_progress = MapAction::Moving(object_key);
                    }
                }

                if response.dragged() && let MapAction::Moving(object_key) = state.map.action_in_progress {
                    let pointer_delta = response.drag_delta();
                    let world_delta = Vec2::new(pointer_delta.x, pointer_delta.y) / state.map.zoom;

                    if world_delta != Vec2::ZERO && let Some(filename) = state.terminal.get_object_filename(object_key) {
                        let args = ModifySceneObjectArgs {
                            target_path: format!("scene/{}", filename),
                            action: ModifyAction::Translate(world_delta),
                        }.format_args();

                        ModifySceneObjectCommand::execute_app(state, &args);
                    }
                }
            ...
            MapTool::Remove => {
                if response.clicked() && let Some(pointer_position) = response.interact_pointer_pos() {
                    ...
                    if let Some(key) = state.scene.key_object_around(world_position, search_radius).map(|kv| kv.0) &&
                    let Some(filename) = state.terminal.get_object_filename(key) {
                        let args = RemoveSceneObjectArgs {
                            target_path: format!("scene/{}", filename),
                        }.format_args();

                        RemoveSceneObjectCommand::execute_app(state, &args);
                    }
                }
            }
            ...
            MapTool::AddReceiver => {
                if response.clicked() && let Some(pointer_position) = response.interact_pointer_pos() {
                    ...
                    let args = AddSceneObjectArgs::Receiver(world_position).format_args();
                    AddSceneObjectCommand::execute_app(state, &args);
                }
            }
            ...
            MapTool::AddEmitter => {
                if response.clicked() && let Some(pointer_position) = response.interact_pointer_pos() {
                    let screen_position = Vec2::new(pointer_position.x, pointer_position.y);
                    let world_position = state.map.screen_to_world(screen_position);
                    let args = AddSceneObjectArgs::Emitter(world_position).format_args();
                    AddSceneObjectCommand::execute_app(state, &args);
                }
            }
                ...
                if !ui.egui_wants_keyboard_input() {
                    ui.input(|input_state: &egui::InputState| {
                        if input_state.focused && input_state.key_pressed(egui::Key::Enter) {
                            let completed_action = std::mem::replace(
                                &mut state.map.action_in_progress,
                                MapAction::None,
                            );

                            if let MapAction::AddingPolygon(Shape::Polygon(vertices, ..)) = completed_action {
                                let args = AddSceneObjectArgs::Wall(vertices).format_args();
                                AddSceneObjectCommand::execute_app(state, &args);
                            }
                        }
