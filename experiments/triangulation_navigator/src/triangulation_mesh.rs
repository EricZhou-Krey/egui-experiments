...
impl AnimatedTriangulationMesh {
    ...
    pub fn update(&mut self, dt: f32, bounds: [f32; 4]) {
        let mut current_vertices: Vec<[f32; 2]> =
            self.mesh.vertices.iter().map(|v: &Vertex| v.pos).collect();

        current_vertices[0] = [bounds[0], bounds[2]];
        current_vertices[1] = [bounds[1], bounds[2]];
        current_vertices[2] = [bounds[1], bounds[3]];
        current_vertices[3] = [bounds[0], bounds[3]];
        ...
        for (i, v) in self.velocities.iter_mut().enumerate() {
            ...
            let mut pos: [f32; 2] = current_vertices[i];
